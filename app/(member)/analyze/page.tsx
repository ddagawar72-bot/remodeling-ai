...
import type { SessionUser, HouseMBTI, CompatibilityResult } from "@/types";
...
export default function AnalyzePage() {
  ...
  const scoreColor = (score: number) =>
    score >= 90 ? "#4a9e6b" : score >= 70 ? "#9e7a52" : score >= 50 ? "#c4a35a" : "#b84040";
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-3xl tracking-[6px] uppercase font-light mb-2"
          style={{ fontFamily: "'Cormorant Garamond',serif" }}>
          우리집 MBTI
        </h1>
        <p className="text-sm" style={{ color: "var(--txm)", letterSpacing: "0.5px" }}>
          사진 한 장으로 우리집의 MBTI를 분석하고, 나와의 궁합을 확인하세요.
        </p>
      </div>
      ...
      <div className="flex items-center justify-between mb-6 px-4 py-3 rounded border text-xs"
        ...
        <span style={{ color: "var(--txm)", letterSpacing: "1px" }}>
          보유 포인트: <strong style={{ color: "var(--ac)" }}>{(user?.points ?? 0).toLocaleString()} pt</strong>
        </span>
      </div>
        ...
        {imageDataUri ? (
          <div className="relative">
            ...
            <button className="absolute top-3 right-3 w-8 h-8 rounded-full flex items-center justify-center text-sm"
              ...
              onClick={(e) => { e.stopPropagation(); setImageDataUri(null); setHouseMbti(null); setCompatibility(null); }}>
              ×
            </button>
          </div>
        ) : (
      ...
      {!houseMbti && (
        <button disabled={!imageDataUri || loading} onClick={handleAnalyze}
          ...
          {loading ? (
            <><span className="inline-block w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin" />{loadingMsg}</>
          ) : "✦ 우리집 MBTI 분석 시작"}
        </button>
      )}
      ...
      {houseMbti && (
        <div className="mt-6 space-y-5">
          ...
          <div className="p-6 rounded-lg border text-center"
            ...
            <div className="text-5xl font-light tracking-widest mb-2"
              style={{ fontFamily: "'Cormorant Garamond',serif", color: "var(--ac)" }}>
              {houseMbti.mbti}
            </div>
          ...
          <div className="p-5 rounded-lg border" style={{ background: "var(--bg2)", borderColor: "var(--bd)" }}>
            ...
            <div className="grid grid-cols-2 gap-2">
              {houseMbti.traits.map((t, i) => (
                <div key={i} className="px-3 py-2 rounded text-xs text-center"
                  style={{ background: "rgba(158,122,82,.06)", border: "1px solid rgba(158,122,82,.12)", color: "var(--tx)" }}>
                  {t}
                </div>
              ))}
            </div>
          ...
          <div className="p-5 rounded-lg border" style={{ background: "var(--bg2)", borderColor: "var(--bd)" }}>
            ...
            <div className="flex gap-2 flex-wrap">
              {houseMbti.colorPalette.map((c, i) => (
                <span key={i} className="px-3 py-1.5 rounded text-xs tracking-widest"
          ...
          {!compatibility && (
            <div className="p-5 rounded-lg border" style={{ background: "var(--bg2)", borderColor: "var(--bd)" }}>
              <div className="text-[9px] tracking-[3px] uppercase mb-3" style={{ color: "var(--ac)" }}>
                나의 MBTI를 선택하면 궁합을 분석해드려요
              </div>
              <div className="grid grid-cols-4 gap-2 mb-4">
                {MBTI_LIST.map((m) => (
                  <button key={m} onClick={() => setUserMbti(m)}
                    className="py-2 rounded text-xs tracking-widest transition-all"
                    style={{
                      background: userMbti === m ? "var(--ac)" : "rgba(158,122,82,.06)",
                      color: userMbti === m ? "var(--bg)" : "var(--tx)",
                      border: `1px solid ${userMbti === m ? "var(--ac)" : "rgba(158,122,82,.12)"}`,
                      fontWeight: userMbti === m ? 600 : 400,
                    }}>
                    {m}
                  </button>
                ))}
          ...
          {compatibility && (
            <div className="space-y-4">
              ...
              <div className="p-6 rounded-lg border text-center" style={{ background: "var(--bg2)", borderColor: "var(--bd)" }}>
                <div className="text-[9px] tracking-[3px] uppercase mb-2" style={{ color: "var(--txm)" }}>
                  {houseMbti.mbti} 집 × {userMbti} 나
                </div>
                <div className="text-6xl font-light mb-1"
                  style={{ fontFamily: "'Cormorant Garamond',serif", color: scoreColor(compatibility.score) }}>
                  {compatibility.score}
                </div>
                ...
                <div className="mt-4 h-2 rounded-full overflow-hidden" style={{ background: "var(--bd)" }}>
                  <div className="h-full rounded-full transition-all duration-1000"
                    style={{ width: `${compatibility.score}%`, background: scoreColor(compatibility.score) }} />
                </div>
              </div>
                ...
                <div className="p-5 rounded-lg border" style={{ background: "var(--bg2)", borderColor: "var(--bd)" }}>
                  ...
                  <ul className="space-y-2">
                    {compatibility.cons.map((c, i) => (
                      <li key={i} className="text-sm flex gap-2" style={{ color: "var(--txm)" }}>
                        <span style={{ color: "var(--dn)" }}>✗</span>{c}
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              {/* 개선 제안 */}
              <div className="p-5 rounded-lg border" style={{ background: "var(--bg2)", borderColor: "var(--bd)" }}>
                <div className="text-[9px] tracking-[3px] uppercase mb-4" style={{ color: "var(--ac)" }}>
                  인테리어 개선 제안
                </div>
                <div className="space-y-4">
                  {compatibility.improvements.map((item, i) => (
                    <div key={i} className="border-l-2 pl-4" style={{ borderColor: "var(--ac)" }}>
                      <div className="text-xs font-medium mb-1" style={{ color: "var(--ac)", letterSpacing: "1px" }}>
                        {item.area}
                      </div>
                      <div className="text-xs mb-1" style={{ color: "var(--txm)" }}>
                        현재: {item.current}
                      </div>
              ...
              {/* 다시 분석 */}
              <button onClick={() => { setHouseMbti(null); setCompatibility(null); setUserMbti(""); setImageDataUri(null); }}
                className="w-full py-3 rounded text-xs tracking-[3px] uppercase transition-all"
                style={{ background: "transparent", color: "var(--txm)", border: "1px solid var(--bd)", fontFamily: "'DM Sans',sans-serif" }}>
                다시 분석하기
              </button>
            </div>
          )}
